...
* Commands
** Shorthand CLI
From project's root install the nix package manager:

#+begin_src sh
./hey install-nix
#+end_src

Install a home-manager profile defined in the flake:

#+begin_src shell
./hey hm-switch .#<USER>@<HOST>
#+end_src
...
* Enable custom shell when login through ssh

Add the following to ~.profile~ to run ~zsh~ automatically upon login:
