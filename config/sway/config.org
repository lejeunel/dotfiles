...
#+begin_src sh :tangle definitions
set $locking ${HOME}/.config/sway/scripts/mylock
#+end_src
...
** Locking
#+begin_src sh :tangle modes/locking
set $mode_locking "\
<span>🔒</span> \
<span foreground='$base06'> \
<span foreground='$base04'>(<b>s</b>)</span>safe \
<span foreground='$base04'>(<b>m</b>)</span>movie \
<span foreground='$base04'>(<b>h</b>)</span>home \
<span foreground='$base04'>(<b>d</b>)</span>download \
<span> -</span>  \
</span>"
#+end_src

#+begin_src sh :tangle modes/locking
mode --pango_markup $mode_locking {
    $bindsym s exec $locking safe
    $bindsym m exec $locking movie
    $bindsym h exec $locking home
    $bindsym d exec $locking download

    # Return to default mode.
    $bindsym Escape mode "default"
}

$bindsym $mod+Shift+w mode $mode_locking
#+end_src
